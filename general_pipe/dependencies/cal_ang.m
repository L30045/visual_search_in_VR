function [ang, v_ang] = cal_ang(mv_test_data,srate,velocity_smooth_win_len,varagin)
if isempty(varagin)
    ang_tolerance = 0.1;
else
    ang_tolerance = varagin{1};
end
...
for v_i = vel_win_len+1:size(mv_test_data,2)-vel_win_len
    if ~isnan(mv_test_data(1,v_i+[-vel_win_len,vel_win_len]))
        nomi = double(mv_test_data(:,v_i-vel_win_len)'*mv_test_data(:,v_i+vel+win+len));
        ...
        if 1-nomi/denomi <= tolerance
            ...
            v_ang(v_i) = ang(v_i)/(0.001*velocity_smooth_win_len)/pi*180; % (degree/sec)
        end
    end
end
